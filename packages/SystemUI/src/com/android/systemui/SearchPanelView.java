       ...
       final Runnable SetLongPress = new Runnable () {
            public void run() {
                if (!mSearchPanelLock) {
                    ...
                    if (!TGActions.isActionKeyEvent(mLongList.get(mTarget))) {
                        performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);
                    }
                    sendAccessibilityEvent(AccessibilityEvent.TYPE_VIEW_LONG_CLICKED);
                    TGActions.processAction(mContext, mLongList.get(mTarget), true);
        ...
        public void onTrigger(View v, final int target) {
            ...
            mTarget = target;
            if (!mLongPress) {
                if (!TGActions.isActionKeyEvent(mIntentList.get(target))) {
                ...
                TGActions.processAction(mContext, mIntentList.get(target), false);
                mHandler.removeCallbacks(SetLongPress);
            }
        }
