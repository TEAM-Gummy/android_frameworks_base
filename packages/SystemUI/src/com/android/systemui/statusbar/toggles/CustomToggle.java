...
public class CustomToggle extends BaseToggle {
    ...
    public final static String[] StockClickActions = {
        GummyConstant.ACTION_NULL.value(),
        GummyConstant.ACTION_NULL.value(),
        GummyConstant.ACTION_NULL.value(),
        GummyConstant.ACTION_NULL.value(),
        GummyConstant.ACTION_NULL.value() };
    ...
    private void startActions() {
        if (mMatchAction) {
            GummyAction.launchAction(mContext, mClickActions[mMatchState]);
        } else {
            GummyAction.launchAction(mContext, mClickActions[mCustomState]);
        }
        shouldCollapse();
        startMagicTricks();
    }
    ...
    final void launchLongOrShort() {
        GummyAction.launchAction(mContext, "**null**".equals(mClickActions[mCustomState])
    ...
    private void startMagicTricks() {
        ...
        String toggleText = GummyHelpers.getProperSummary(mContext,
        ...
        if (iconUri != null && iconUri.length() > 0) {
            ...
            myIcon = GummyHelpers.getIconImage(mContext, "**null**".equals(mClickActions[mCustomState])
                    ? mLongActions[mCustomState] : mClickActions[mCustomState]);
        }
        setLabel(toggleText);
        setIcon(myIcon);
    ...
    public boolean onLongClick(View v) {
        GummyAction.launchAction(mContext, mLongActions[mCustomState]);
        switch (mCollapseShade) {
            case NO_COLLAPSE:
            case ON_CLICK:
                break;
